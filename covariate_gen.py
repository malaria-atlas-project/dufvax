...
clipped_pete_mask = grid_convert(grid_convert(pete_mask,'y-x+','x+y+')[lon_min_i/5.:lon_max_i/5.,lat_min_i/5.:lat_max_i/5.],'x+y+','y-x+')
...
for cmph in cmph_covariates:
    ...
    lon_,lat_,data = map_utils.CRU_extract('/mnt','%s'%cmph, zip=False)
    lon_.sort()
    lat_.sort()
