...
\begin{eqnarray*}
    ...
    C(x_i,t_i;x_j,t_j)=\tau^2\gamma(0)\frac{(\Delta x)^{\gamma(\Delta t)}K_{\gamma(\Delta t)}(\Delta x)}{2^{\gamma(\Delta t)-1}\Gamma(\gamma(\Delta t)+1)},\\ \gamma(\Delta t) = \frac{1}{2\rho+2(1-\rho)\left[(1-\upsilon)e^{-|\Delta t|/\phi_t}+\upsilon\cos(2\pi \Delta t)\right]},\\ \Delta t = |t_i-t_j|
    p_v(x)=\textup{logit}^{-1}(f_{v}(x))
\end{eqnarray*} 
A space-time process is more appropriate than a space-only process due to the seasonal and rapid secular changes seen in malaria transmission dynamics \cite{blah}. 
